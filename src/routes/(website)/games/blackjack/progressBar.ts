...
export const addAutoplayProgressListeners = (
	...
	const progressBarNode = <HTMLElement>progressNode.querySelector('.embla__progress__bar');
	...
	emblaApi.on('autoplay:timerset', startProgress).on('autoplay:timerstopped', stopProgress);

	return () => {
		emblaApi.off('autoplay:timerset', startProgress).off('autoplay:timerstopped', stopProgress);
	};
};
